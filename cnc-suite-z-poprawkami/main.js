/* CNC Suite — proces główny Electron.
   Jedno okno z kartami, w którym osadzone są trzy aplikacje:
     • Menedżer programów CNC   (manager.html)
     • Generator G-code / NC    (generator.html)
     • Kalkulator CNC           (kalkulator.html)
   Obsługa systemowa (pliki, okna podglądu, widget, zapis .nc) jest połączeniem
   procesów głównych obu oryginalnych aplikacji — bez zmian w ich zachowaniu. */
...
/* ── rejestr ramek ────────────────────────────────────────────────────────
   Aplikacje działają w ramkach (iframe) powłoki, a webContents.send() trafia
   wyłącznie do ramki głównej. Każda strona melduje się przez preload, dzięki
   czemu wiadomości (potwierdzenie zamknięcia, widget, okno podglądu) trafiają
   do właściwej karty. */
const frames = new Map();            // nazwa strony -> WebFrameMain

ipcMain.on("register-frame", (e, name) => {
  if (typeof name === "string" && e.senderFrame) frames.set(name, e.senderFrame);
});

function frameOf(name) {
  const f = frames.get(name);
  if (!f) return null;
  try { if (f.detached) { frames.delete(name); return null; } } catch { frames.delete(name); return null; }
  return f;
}

/* wysyła do karty menedżera; gdy jeszcze się nie zgłosiła — do powłoki */
function sendToManager(channel, ...args) {
  const f = frameOf("manager");
  if (f) { try { f.send(channel, ...args); return; } catch {} }
  if (win && !win.isDestroyed()) win.webContents.send(channel, ...args);
}

/* przełącza powłokę na wskazaną kartę — okna dialogowe menedżera są rysowane
   w jego karcie, więc muszą być widoczne, zanim je pokażemy */
function activateTab(tab) {
  if (win && !win.isDestroyed()) win.webContents.send("suite-activate-tab", tab);
}
...
function createWindow() {
  ...
  win.on("close", (e) => {
    ...
    win.show();
    win.focus();
    activateTab("manager");            // pytanie rysuje karta menedżera
    sendToManager("app-ask-close");
  });
...
function createTray() {
  ...
  try {
    ...
    tray.setToolTip("CNC Suite z poprawkami");
...
function createWidgetWindow() {
  ...
  widgetWin = new BrowserWindow({
    ...
    title: "Szukaj — CNC Suite z poprawkami",
...
function notifyWidgetState(on) { sendToManager("widget-state", on); }
...
/* zapis pliku .nc przez natywne okno „Zapisz jako" (generator G-code) */
function attachDownloadHandler(w) {
  w.webContents.session.on("will-download", (event, item) => {
    const name = item.getFilename() || "program.nc";
    const savePath = dialog.showSaveDialogSync(w, {
...
/* akcje z okna podglądu (Zapisz / Usuń / Pobierz) przekazywane do karty menedżera.
...
ipcMain.on("detail-action", (e, msg) => {
  if (win && !win.isDestroyed()) {
    ...
    sendToManager("detail-action", msg);
  }
  if (msg && msg.close) {
    const w = BrowserWindow.fromWebContents(e.sender);
    if (w) w.close();
...
ipcMain.on("widget-query", (e, query) => {
  sendToManager("widget-live", query);
});
...
ipcMain.on("widget-submit", (e, query) => {
  if (win && !win.isDestroyed()) {
    ...
    win.show();
    win.focus();
    activateTab("manager");            // wyniki pokazuje karta menedżera
    sendToManager("widget-go", query);
  }
});
